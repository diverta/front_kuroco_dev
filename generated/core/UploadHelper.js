...
export function generateUploader(gcsHost, accessToken, chunkSize) {
    ...
    flow.uploadAsync = function () {
        return new Promise(function (res, rej) {
            flow.on('fileSuccess', function (file, message, chunk) { return res("files/temp/" + file.uniqueIdentifier + "__rcms_gcs_upload_chunk_number__" + chunk.offset); });
...
export function getTarget(uri) {
    return function (flowFile, flowChunk) { return uri + "/files/temp/" + flowFile.uniqueIdentifier + "__rcms_gcs_upload_chunk_number__" + flowChunk.offset; };
}
